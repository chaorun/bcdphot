...
def match_cats(df1, df2, tol=1/3600.):
    cols1 = [i.lower() for i in df1.columns]
    cols2 = [i.lower() for i in df2.columns]
    try:
        ra1ix = cols1.index('ra')
        dec1ix = cols1.index('dec')
        ra2ix = cols2.index('ra')
        dec2ix = cols2.index('dec')
        idx1, idx2 = match(df1.iloc[:,ra1ix], df1.iloc[:,dec1ix],
            df2.iloc[:,ra2ix], df2.iloc[:,dec2ix], tol=tol)
    except:
        raise KeyError("DataFrame missing RA/Dec")
    return idx1, idx2


def match(ra1, dec1, ra2, dec2, tol=1/3600.):
    assert len(ra1) == len(dec1)
    assert len(ra2) == len(dec2)
    if len(ra1) < len(ra2):
        idx1, idx2, ds = spherematch(ra1, dec1,
            ra2, dec2, tolerance=tol)
    else:
        idx2, idx1, ds = spherematch(ra2, dec2,
            ra1, dec1, tolerance=tol)
    return idx1, idx2
